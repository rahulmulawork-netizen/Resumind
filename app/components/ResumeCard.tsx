...
const ResumeCard = ({ resume }: { resume: Resume & { imageFilePath?: string } }) => {
    const { id, companyName, jobTitle, feedback, imagePath, imageFilePath } = resume;
    const { fs } = usePuterStore();
    const [resumeUrl, setResumeUrl] = useState('');

    useEffect(() => {
        const loadResume = async () => {
            const path = imagePath || imageFilePath;
            if(!path) return;
